...
while not done:
    ...
    if (printMenu):
        ...
        print(format("b - breakers engage/disengage", "<34s"), end="    ")
        print(format("\n\tg - generator engage/disengage", "<34s"), end="     ")
    ...
    if (r == 't'):
        if (throttleAdvance):
            ...
            if (engineRunning):
                ...
                if (generatorEngage):
                    print("The generator disengages.")
                    generatorEngage = False
                    if (lightsOn):
                        print("The breakers disengage and the lights go out.")
                        lightsOn = False
    ...
    if (r == 'g'):
        if (generatorEngage):
            generatorEngage = False
            print("You disengage the generator.")
            if (lightsOn):
                print("The breakers disengage and the lights turn off.")
                lightsOn = False
        elif (throttleAdvance and engineRunning):
            print("You engage the generator, and it starts to generate electricity.")
            generatorEngage = True
            if (engineRunning):
                if (not engineRunning):
                    print("The engine sputters and dies, and the generator shuts off.")
                    engineRunning = False
                    generatorEngage = False
        else:
            print("The generator turns over but does not start.")
            generatorEngage = False
            if (engineRunning):
                print("The engine sputters and dies.")
                engineRunning = False
    ...
    if (r == 'b'):
        if (lightsOn):
            lightsOn = False
            print("You disengage the breakers and the lights shut off.")
        elif (generatorEngage):
            print("You engage the breakers and the lights turn on.")
            lightsOn = True
            if (not generatorEngage):
                print("Without electricity, the lights shut off.")
                lightsOn = False
        else:
            print("You try to engage the breakers and nothing happens.")
            lightsOn = False
